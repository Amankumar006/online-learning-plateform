...
export default function SingleExerciseSolver({ exercise, userId, onSolved, lessonTitle, initialResponse = null }: SingleExerciseSolverProps) {
    ...
    useEffect(() => {
        ...
        // Code exercises - keep editor empty so users build from scratch
    ...
    const renderAnswerArea = () => {
        switch (exercise.type) {
            ...
            case 'code':
                const codeExercise = exercise as CodeExercise;
                return (
                    <div className="space-y-4">
                        <div id="code-exercise-instructions" className="sr-only">
                            Exercise: {codeExercise.description}. Write your {codeExercise.language} code solution below.
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 bg-muted/30 rounded-t-lg border border-b-0">
                            <div className="flex items-center gap-2">
                                <Code className="h-4 w-4 text-primary" />
                                <span className="text-sm font-medium capitalize">{codeExercise.language} Solution</span>
                                <Badge variant="outline" className="text-xs">
                                    {isAnswered ? 'Submitted' : 'Draft'}
                                </Badge>
                            </div>
                        ...
                        <div className="border border-t-0 rounded-b-lg overflow-hidden">
                            <CodeEditor
                                value={longFormAnswer || codeExercise.starterCode || ''}
                                onValueChange={setLongFormAnswer}
                                disabled={isAnswered || isGrading}
                                language={codeExercise.language}
                                placeholder={`// Write your ${codeExercise.language} solution here...\n// Run your code to test it`}
                                aria-label={`Code editor for ${codeExercise.language} exercise`}
                                aria-describedby="code-exercise-instructions"
                                showRunButton={true}
                                onRunCode={handleRunCode}
                                isExecuting={isExecuting}
                            />

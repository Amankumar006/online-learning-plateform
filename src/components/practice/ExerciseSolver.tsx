...
export default function ExerciseSolver({
  ...
  useEffect(() => {
    ...
    if (exercise?.type === 'fill_in_the_blanks') {
        const blankCount = exercise.correctAnswers.length;
  ...
  const handleAnswerSubmit = async () => {
    ...
    try {
        await saveExerciseAttempt( userId, lessonTitle, exercise, submittedAnswer, correct, score, aiFeedback?.feedback, imageDataUri );
         if (exercise.type !== 'long_form') {
  ...
  const handleMathGraded = async (result: GradeMathSolutionOutput, studentSolution: string) => {
    ...
    onAnswered(result.isSolutionCorrect);

    try {
        await saveExerciseAttempt( userId, lessonTitle, exercise, studentSolution, result.isSolutionCorrect, result.overallScore, result, null );
    } catch (error) {
    ...
    switch (exercise.type) {
        ...
        case 'long_form':
            ...
            if (lfExercise.category === 'math') {
                return ( <MathSolutionGrader exercise={lfExercise} onGraded={handleMathGraded} isAnswered={isAnswered} initialFeedback={feedback as GradeMathSolutionOutput || undefined} initialSolution={longFormAnswer} /> );
            }
            return (
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        {exercise.hint && (
            <details className="group">
                <summary className="cursor-pointer text-sm text-muted-foreground inline-flex items-center gap-2">
                    <Lightbulb className="h-4 w-4"/> Show Hint
                </summary>
                <div className="mt-2 text-sm p-4 border rounded-md bg-secondary/50 animate-in fade-in-50">
      ...
      {!isAnswered && (
        <div className="flex justify-end pt-4">
          <Button onClick={handleAnswerSubmit} disabled={isGrading || isSimulating || (exercise.type === 'long_form' && exercise.category === 'math')}>

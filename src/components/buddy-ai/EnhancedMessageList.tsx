...
// Helper to extract text content from SDK message (supports both legacy 'content' and new 'parts' format)
const getMessageContent = (message: any): string => {
    // Legacy format: direct content string
    if (typeof message.content === 'string') {
        return message.content;
    }
    // New SDK format: parts array with TextUIPart objects
                                ...
                                <div className={cn(
                                    ...
                                    {!isUser && (
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            {getMessageContent(message).includes('🌐 **Live Web Search Results**') && (
                                    ...
                                    <div className={cn(
                                        ...
                                        {isUser ? (
                                            <p className="m-0">{getMessageContent(message)}</p>
                                        ) : (
                                            <div className={cn(
                                                "prose prose-sm max-w-none",
                                                message.isError ? "prose-red dark:prose-red" : "dark:prose-invert"
                                            )}>
                                                <FormattedContent content={getMessageContent(message)} />
                                            </div>
                                        )}
                                    </div>
                                            ...
                                            {!message.isError && (
                                                <>
                                                    <Button
                                                        ...
                                                        onClick={() => onPlayAudio(getMessageContent(message), index)}
                                                    >

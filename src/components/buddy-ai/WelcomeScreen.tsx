    ...
    return (
        <div className="flex h-full flex-col items-center justify-center p-4">
            <div className="w-full max-w-2xl mx-auto text-center">
                <div className={cn("p-4 bg-background rounded-full inline-block mb-4 border shadow-sm", activePersona?.color)}>
                    {activePersona?.icon}
                </div>
                ...
                <h2 className="text-lg md:text-xl text-muted-foreground mb-4">{activePersona?.description}</h2>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                            {personalizedPrompts.slice(0, 4).map((prompt, index) => (
                                <Card 
                                    key={index} 
                                    className="p-3 cursor-pointer hover:bg-muted/50 transition-colors border-primary/20 bg-gradient-to-br from-primary/5 to-accent/5"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 text-left">
                    {getPersonaSuggestions().map((suggestion, index) => (
                        <Card key={index} className="p-4 flex flex-col items-start gap-2 cursor-pointer hover:bg-muted transition-colors" onClick={() => onSendSuggestion(suggestion.action)}>
                            {suggestion.icon}

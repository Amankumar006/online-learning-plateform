...
-   **Adaptive Exercises:** The platform provides a rich exercise solver that adapts to the content type. It includes standard solvers for multiple-choice and true/false questions, alongside advanced, AI-powered solvers for specialized subjects.
-   **Interactive Code Solver:** For coding exercises, users are presented with a full code editor. They can write and submit their code to an AI simulation engine (`/src/ai/flows/simulate-code-execution.ts`), which provides console output, error analysis, code suggestions, and Big O complexity analysis in a convenient tabbed view.
-   **Step-by-Step Math Grading:** For mathematical long-form questions, users can input their solution using a rich LaTeX editor. The `grade-math-solution.ts` AI flow evaluates the entire solution, providing a final score, overall feedback, and a detailed step-by-step analysis of where mistakes were made.
-   **General Long-Form Grading:** For other long-form questions, users can submit typed text and/or an image of handwritten work. The `grade-long-form-answer.ts` flow evaluates the submission against predefined criteria.
...
-   **AI-Powered Visual Canvas:** A digital whiteboard (`/dashboard/canvas`) where users can draw diagrams, write equations, or create concept maps. An integrated AI assistant can "solve" or "explain" any selection on the canvas, providing step-by-step solutions for math problems, explaining scientific diagrams, analyzing logic flows, and more. This feature supports a wide range of academic and technical content, turning the canvas into an interactive problem-solving space.

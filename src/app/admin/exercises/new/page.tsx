...
function NewExerciseContent() {
  ...
  const handleGenerateFromLesson = async () => {
    ...
    try {
        ...
        if (results.length === 0) {
            setGenerationFailed(true);
            toast({ variant: "destructive", title: "AI Generation Failed", description: "The AI could not generate exercises for this lesson. Please try a different lesson or adjust the parameters." });
        } else {
            setGeneratedExercises(results);
            toast({ title: "Success!", description: `Generated ${results.length} new exercises. Please review and save.` });
        }

    } catch (error) {
        console.error(error);
        setGenerationFailed(true);
        toast({ variant: "destructive", title: "AI Error", description: "An unexpected error occurred during generation." });
    } finally {
  ...
  const handleGenerateCustom = async () => {
    ...
    try {
      ...
      if (!generatedExercise) {
          setGenerationFailed(true);
          toast({ variant: "destructive", title: "AI Generation Failed", description: "The AI could not generate an exercise for this prompt. Please try rephrasing your request." });
      } else {
                ...
                <div className="text-center text-muted-foreground py-8">
                  {generationFailed ? (
                     <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm">The AI could not generate exercises for this input. Please try a different lesson or adjust your prompt.</p>
                     </div>
                  ) : (
                    <p>Exercises will appear here after generation.</p>
                  )}
                </div>
              )}
            </div>
